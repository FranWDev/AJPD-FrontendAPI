...
import org.dubini.frontend_api.client.FeaturedClient;
...
public class FeaturedService implements CacheWarmable {
     ...
     */
    private CircuitBreaker getFeaturedCircuitBreaker() {
        return circuitBreakerRegistry.circuitBreaker("FeaturedCircuitBreaker",
    ...
    public Mono<Void> warmUpCache() {
        ...
        return FeaturedClient.get()
                .transformDeferred(CircuitBreakerOperator.of(getFeaturedCircuitBreaker()))
                .doOnNext(data -> {
                    cache.put(CACHE_KEY, data);
                    log.info("Cache warmed up with {} featured items", data.size());
                })
                .doOnSuccess(v -> {
    ...
    public Mono<List<PublicationDTO>> get() {
        ...
        if (cached != null && !cached.isEmpty()) {
            log.info("✓ Returning {} featured publications from in-memory cache", cached.size());
        ...
        return FeaturedClient.get()
                .transformDeferred(CircuitBreakerOperator.of(getFeaturedCircuitBreaker()))
                .doOnNext(Featured -> {
                    cache.put(CACHE_KEY, Featured);
                    log.info("Cache updated with {} featured items", Featured.size());
                    if (cacheManager instanceof PersistentCaffeineCacheManager pcm) {
                        pcm.saveCache(CACHE_NAME);
                    }
                })
    ...
    private Mono<List<PublicationDTO>> fallbackFromDisk(Cache cache) {
        if (cacheManager instanceof PersistentCaffeineCacheManager pcm) {
            ...
            List<PublicationDTO> cached = cache.get(CACHE_KEY, List.class);
            if (cached != null && !cached.isEmpty()) {
                log.info("✓ Returning {} featured items from disk cache", cached.size());
                // Repoblar la caché en memoria
                cache.put(CACHE_KEY, cached);
                log.info("In-memory cache repopulated with {} featured items from disk", cached.size());
